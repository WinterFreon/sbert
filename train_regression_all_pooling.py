...
def train_for_pooling(pooling, train_data, val_data, device):
    wandb.init(project="sentence-bert-project", name=f"regression-{pooling}")

    model = SentenceBERT(pooling=pooling).to(device)
    tokenizer = model.tokenizer
    model.load_state_dict(torch.load(f"my_best_sbert_model_{pooling}.pth", map_location=device))
    ...
    for epoch in range(1):
        ...
        for batch in tqdm(train_loader, desc=f"[{pooling}] Epoch {epoch+1} Training"):
            batch = {k: v.to(device) for k, v in batch.items()}
            u = model(batch["input_ids1"], batch["attention_mask1"])
            v = model(batch["input_ids2"], batch["attention_mask2"])
        ...
        print(f"[{pooling}] Epoch {epoch+1} Loss: {avg_train_loss:.4f}")
        print(f"[{pooling}] Validation Spearman: {spearman:.4f}")
    ...
    torch.save(model.state_dict(), f"sbert_regression_{pooling}.pth")
    print(f"✅ Saved: sbert_regression_{pooling}.pth")
    wandb.finish()
...
def main():
    ...
    for pooling in poolings:
        train_for_pooling(pooling, train_data, val_data, device)
